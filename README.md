...
[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=apchavez_net-vue&metric=security_rating)](https://sonarcloud.io/summary/new_code?id=apchavez_net-vue)

# .NET + Vue Fullstack K8s

Monorepo fullstack de gestión de productos: backend **ASP.NET Core Web API** siguiendo Clean/Hexagonal Architecture, frontend **Vue 3** (Composition API, Vuetify), eventos de dominio con Kafka, rate limiting y caché de lectura respaldados por Redis, persistencia en PostgreSQL, y despliegue en Kubernetes (local con `kind` o real) basado en Helm con observabilidad Prometheus/Grafana.

Este repo es uno de los **cinco hermanos** del dominio de Gestión de Productos, junto a [quarkus-react](https://github.com/apchavez/quarkus-react), [spring-webflux-angular](https://github.com/apchavez/spring-webflux-angular), [spring-mvc-angular](https://github.com/apchavez/spring-mvc-angular) y [spring-jpa-native](https://github.com/apchavez/spring-jpa-native) — con los primeros tres comparte el mismo dominio de negocio (`sku`/`name`/`description`/`category`/`price`/`stock`/`active`), prácticamente los mismos endpoints REST (11 aquí y en quarkus-react, cuyo import CSV es síncrono; 12 en los dos hermanos Spring, que además exponen `GET /import/{jobExecutionId}` para consultar el estado del import asíncrono), mismo topic de Kafka `product-events` y mismos tipos de evento, mismas reglas de rate limiting en Redis (ventana fija de 100 req/60s en endpoints de escritura, 429 + `Retry-After`), mismo comportamiento de caché cache-aside en Redis sobre el listado de productos activos (TTL de 5 min, fail-open), distinto stack de backend/frontend. Con spring-jpa-native comparte el dominio de gestión de productos con un enfoque distinto, centrado en JPA/Hibernate (relaciones de entidades, fix de N+1, bloqueo optimista, auditoría) y un frontend móvil en React Native en vez de web.
...
> **Dev sin instalar el toolchain:** `./docker/dev.sh api-test` / `web-test` corren dentro del mismo contenedor que usa CI, sin necesitar .NET/Node instalados localmente (`./docker/dev.sh` sin argumentos muestra las opciones).
...
| Infraestructura | Helm, Docker, Kubernetes (local `kind` o real), GitHub Actions |
...
- `chart/` — Chart de Helm que despliega `product-api` + `product-web` + Postgres + Redis + Kafka (KRaft, SASL-SCRAM) + Prometheus/Grafana. Corre sobre cualquier cluster K8s, local (`kind`) o real — no hay un cluster activo para este proyecto.
- `.github/workflows/deploy.yml` — `workflow_dispatch` manual. Levanta un cluster `kind` efímero dentro del propio runner, despliega el chart (con `monitoring.enabled=false`, ya que el Prometheus Operator no está instalado en el cluster efímero), corre un smoke test real contra `/health/ready` y la home del frontend, y destruye el cluster al finalizar. No depende de ningún cluster ni `KUBECONFIG` externo.
- Para levantar el chart contra un `kind` local en tu máquina: `kind create cluster && helm upgrade --install product-api ./chart -n product-api --create-namespace --set api.image.tag=latest --set web.image.tag=latest --set monitoring.enabled=false --set secrets.dbUser=... --set secrets.dbPassword=... --set secrets.kafkaPassword=... --set secrets.redisPassword=...` (borra el cluster con `kind delete cluster` cuando termines).
...
| [spring-jpa-native](https://github.com/apchavez/spring-jpa-native) | API de Gestión de Productos que demuestra JPA/Hibernate (relaciones de entidades, fix de N+1, bloqueo optimista, auditoría) con frontend móvil en React Native |
