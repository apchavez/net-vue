...
public sealed class AuthController(DemoUserStore userStore, JwtTokenService tokenService, ILogger<AuthController> logger) : ControllerBase
{
    ...
    public IActionResult Login([FromBody] LoginRequestDto request)
    {
        ...
        if (roles is null)
        {
            logger.LogWarning("Login failed — username={Username}", request.Username);
        ...
        logger.LogInformation("Login succeeded — username={Username}, roles={Roles}", request.Username, string.Join(",", roles));
