...
[Route("api/v1/auth")]
[AllowAnonymous]
public sealed class AuthController(DemoUserStore userStore, JwtTokenService tokenService, ILogger<AuthController> logger) : ControllerBase
{
    /// <summary>
    ...
    public IActionResult Login([FromBody] LoginRequestDto request)
    {
        ...
        if (roles is null)
        {
            logger.LogWarning("Login failed — username={Username}", LogSanitizer.Sanitize(request.Username));
        ...
        logger.LogInformation("Login succeeded — username={Username}, roles={Roles}", LogSanitizer.Sanitize(request.Username), string.Join(",", roles));
