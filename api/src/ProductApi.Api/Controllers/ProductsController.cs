...
public sealed class ProductsController(IProductService productService, ILogger<ProductsController> logger) : ControllerBase
{
    ...
    {
        logger.LogInformation("Creating product — sku={Sku}", LogSanitizer.Sanitize(dto.Sku));
    ...
    {
        logger.LogInformation("Updating product — id={Id}", id);
    ...
    public async Task<IActionResult> Import(IFormFile? file, CancellationToken ct)
    {
        ...
        logger.LogInformation("Starting product CSV import — fileName={FileName}, sizeBytes={Size}", file.FileName, file.Length);
        ...
        logger.LogInformation("Finished product CSV import — imported={Imported}, errors={Errors}", imported, errors.Count);
